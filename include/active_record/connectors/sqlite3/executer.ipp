...
namespace active_record {
    ...
    public:
        ...
        public:
            ...
            active_record::expected<std::reference_wrapper<const ResultType>, active_record::string> operator*();
            iterator& operator++();
        };
    ...
    public:
        ...
        active_record::expected<void, active_record::string> execute();
    };
    ...
    inline active_record::expected<void, active_record::string> sqlite3_connector::executer<void>::execute(){
        const auto status = sqlite3_step(stmt);
        if(status != SQLITE_DONE){
            return active_record::make_unexpected(active_record::string{ sqlite3_errmsg(sqlite3_db_handle(stmt)) });
        }
        else{
    ...
    template<typename ResultType>
    inline active_record::expected<std::reference_wrapper<const ResultType>, active_record::string> sqlite3_connector::executer<ResultType>::iterator::operator*(){
