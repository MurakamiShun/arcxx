...
namespace arcxx {
    ...
    struct model {
        struct schema {
            ...
            [[nodiscard]] static arcxx::string to_sql(decltype(abort_if_exists));
            template<is_connector Connector>
            [[nodiscard]] static arcxx::string to_sql();
        };
        ...
        [[nodiscard]] static constexpr auto column_names() noexcept {
            return std::apply(
                []<is_attribute... Attrs>([[maybe_unused]]Attrs...) constexpr noexcept { return std::array<const arcxx::string_view, sizeof...(Attrs)>{(Attrs::column_name)...}; },
        ...
        template<is_attribute Attr>
        [[nodiscard]] static auto order_by(const arcxx::order = arcxx::order::asc);
